...
// Source file: home/kevintan/driver-mobile-app/app/src/main/proto/gtfs-realtime.proto at 80:1
...
public final class FeedEntity extends Message<FeedEntity, FeedEntity.Builder> {
  ...
  public FeedEntity(String id, Boolean is_deleted, TripUpdate trip_update, VehiclePosition vehicle, Alert alert) {
    this(id, is_deleted, trip_update, vehicle, alert, ByteString.EMPTY);
  }

  public FeedEntity(String id, Boolean is_deleted, TripUpdate trip_update, VehiclePosition vehicle, Alert alert, ByteString unknownFields) {
  ...
  private static final class ProtoAdapter_FeedEntity extends ProtoAdapter<FeedEntity> {
    ...
    public int encodedSize(FeedEntity value) {
      return ProtoAdapter.STRING.encodedSizeWithTag(1, value.id)
          + (value.is_deleted != null ? ProtoAdapter.BOOL.encodedSizeWithTag(2, value.is_deleted) : 0)
          + (value.trip_update != null ? TripUpdate.ADAPTER.encodedSizeWithTag(3, value.trip_update) : 0)
          + (value.vehicle != null ? VehiclePosition.ADAPTER.encodedSizeWithTag(4, value.vehicle) : 0)
          + (value.alert != null ? Alert.ADAPTER.encodedSizeWithTag(5, value.alert) : 0)
    ...
    public void encode(ProtoWriter writer, FeedEntity value) throws IOException {
      ...
      if (value.is_deleted != null) ProtoAdapter.BOOL.encodeWithTag(writer, 2, value.is_deleted);
      if (value.trip_update != null) TripUpdate.ADAPTER.encodeWithTag(writer, 3, value.trip_update);
      if (value.vehicle != null) VehiclePosition.ADAPTER.encodeWithTag(writer, 4, value.vehicle);
      if (value.alert != null) Alert.ADAPTER.encodeWithTag(writer, 5, value.alert);
